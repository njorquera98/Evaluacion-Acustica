...
float db = 0.0; //valor medido en dB con dos decimales
int contador=0; //variable que aumentamos hasta el valor que queremos cada medicion
...
void loop() {
  if (millis() - lastQuery >= QUERY_INTERVAL) {
    ...
    delay(200); // Esperar respuesta
    db = readSensorResponse();
    if(contador==5){//el contador se usa para determinar que valor de db queremos enviar para guardar en dB
    ...
    delay(1000); // Control de frecuencia de medición
  }
}
